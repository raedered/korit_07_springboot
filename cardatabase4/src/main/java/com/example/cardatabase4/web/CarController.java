...
public class CarController {
    ...
    public ResponseEntity<Car> getCarById(@PathVariable Long id) {
        return carService.getCarById(id)
                .map(car -> ResponseEntity.ok().body(car))  // 차량 정보가 있으면 200 OK 를 반환할것
    ...
    public ResponseEntity<Car> updateCar(@PathVariable Long id, @RequestBody Car carDetails) {
        return carService.updateCar(id, carDetails)
                .map(updateCar -> ResponseEntity.ok().body(updateCar)) // 수정 성공 시에는 200OK
                .orElse(ResponseEntity.notFound().build());     // 수정할 차량 id검색 실패시 404 Not Found
    }
}
