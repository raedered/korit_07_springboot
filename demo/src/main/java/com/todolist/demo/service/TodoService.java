...
import com.todolist.demo.dto.TodoRequestRecode;
...
public class TodoService {
    ...
    // 본인 확인 로직을 추가. 그러니까 To-do 객체 내에 있는 User와, 지금 요청을 보내는 애가 동일한지를 확인해야함.
    private void checkOwnership(Todo todo) throws AccessDeniedException {
        if(!todo.getUser().equals(getCurrentUser())){
            throw new AccessDeniedException("해당 todo에 접근할수 없습니다.");
        }
    }
    ...
    // Record version
    public Todo createTodo2(TodoRequestRecode todoRequestRecode) {
        User currentUser = getCurrentUser();
        Todo newTodo = new Todo(todoRequestRecode.content(), currentUser);
    ...
    public Todo updateTodoContent(Long id, TodoRequestDto updateDto) throws AccessDeniedException {
        Todo todo = todoRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("해당 id 를 가진 todo가 없습니다. id: " + id));
        checkOwnership(todo);
